...
function test(request: protos.conformance.ConformanceRequest): Result {
  if (request.messageType === FailureSet.name) {
    ...
    return {
      protobufPayload: FailureSet.encode(failureSet, new Writer()).finish(),
    };
  }
  ...
  switch (request.requestedOutputFormat) {
    case 1: // PROTOBUF
      return {
        ...
        protobufPayload: payloadType.encode(payload as any, new Writer()).finish(),
      };
...
function testIo(
  ...
  const serializedResponse = ConformanceResponse.encode(response, new Writer()).finish();
