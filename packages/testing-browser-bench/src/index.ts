...
// @ts-ignore
import partialSketchPayloadUrl from './payloads/partialSketch.bin.br.raw'
...
const setupBenchmarkSuite = (btn: HTMLButtonElement, textDiv: HTMLDivElement) => {
    ...
    btn.addEventListener('click', async () => {
        ...
        const _partialSketchMessagePayload = await fetch(partialSketchPayloadUrl).then(resp => resp.blob()).then(blob => blob.arrayBuffer()).then(bytes => new Uint8Array(bytes)).then(bytes => brotli.decompress(bytes))
